    ...
    <div class="flex flex-col items-center justify-center w-full p-4">
        ...
        @if(isset($checkout) && $checkout)
            <form action="" method="POST">
                @csrf
                <button type="submit" class="mt-4 px-4 py-2 bg-red-500 text-white rounded-md focus:outline-none hover:bg-red-600">Remove from Cart</button>
            </form>
        @else
            <!-- add to card button with form -->
            <form action="{{ }}" method="POST">
                @csrf
                <button type="submit" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-md focus:outline-none hover:bg-blue-600">Add to Cart</button>
            </form>
        @endif
    </div>
</div>
